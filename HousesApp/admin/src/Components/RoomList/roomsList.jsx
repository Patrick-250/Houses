...
const RoomsList = () => {
    ...
    const fetchRooms = async () => {
      if (houseId.house_id) {
        const res = await axios.get(
          `http://172.16.161.30:3000/api/users/${houseId.house_id}`
        );
        setUser(res.data);
  ...
  const Welcome = (
    <div className="wel" style={{ backgroundColor: "white" }}>
      
      <span className="Message"     style={{ fontSize: "30px",color:"black"}}>
        Welcome to QLI Houses Admin Panel
      </span>
    </div>
  ...
  const handleDelete = async () => {
    try {
      const res = await axios.delete(
        `http://172.16.161.30:3000/api/users/delete/${houseId.house_id}`
      );
      console.log(res);
