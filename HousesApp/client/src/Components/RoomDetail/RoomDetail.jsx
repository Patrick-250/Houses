...
const RoomDetail = () => {
  ...
  useEffect(() => {
    const fetchRoom = async () => {
      try {
        const res = await axios.get(`http://172.16.161.30:3000/api/rooms/${id}`);
        setData(res.data);
    ...
    const getSchedules = async () => {
      try {
        const res = await axios.get(
          `http://172.16.161.30:3000/api/schedules/${id}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "http://172.16.161.30:3000/api/schedules",
        data,
        {
          headers: { Authorization: newToken },
        }
...
const Schedules = (props) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await axios.put(
      `http://172.16.161.30:3000/api/schedules/${props.sch._id}`,
      data,
      {
        headers: { Authorization: newToken },
      }
  ...
  useEffect(() => {
    const getSchedules = async () => {
      try {
        const res = await axios.get(
          `http://172.16.161.30:3000/api/schedules/one/${props.sch._id}`
        );
        const formattedSchedules = res.data.map((schedule) => ({
          title: schedule.title,
          start: new Date(schedule.start), // Ensure Date objects
